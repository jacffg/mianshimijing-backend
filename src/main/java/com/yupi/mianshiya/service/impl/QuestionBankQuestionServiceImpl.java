...
//        }
    ...
    public void batchRemoveQuestionsFromBank(List<Long> questionIdList, long questionBankId) {
        ...
        validQuestionIdList = validQuestionIdList.stream().filter(existQuestionIdSet::contains).collect(Collectors.toList());
        ThrowUtils.throwIf(CollUtil.isEmpty(validQuestionIdList), ErrorCode.PARAMS_ERROR, "所有题目不存在于题库中");
        ...
        for (int i = 0; i < totalQuestionListsize; i += batchSize) {
            ...
            subList.forEach(questionId -> {
                ...
                LambdaQueryWrapper<QuestionBankQuestion> questionLambdaQueryWrapper = Wrappers.lambdaQuery(QuestionBankQuestion.class)
                        ...
                        .eq(QuestionBankQuestion::getQuestionBankId, questionBankId);
                try {
                    boolean result = this.remove(questionLambdaQueryWrapper);
                } catch (DataIntegrityViolationException e) {
                    log.error("数据库唯一键冲突或违反其他完整性约束, 错误信息: {}", e.getMessage());
                    throw new BusinessException(ErrorCode.OPERATION_ERROR, "题目已存在于该题库，无法重复添加");
                } catch (DataAccessException e) {
